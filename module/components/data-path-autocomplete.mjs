import { createRef, html, LitElement, ref, when } from "../lib/lit-all.min.js";
	...
	render() {
		return html`
			<input
				...
				@keydown=${() => this.#recalculateDisplayedDataPaths()}
				?disabled=${this.disabled}
			>

			${when(this._isDropdownOpen && !this.disabled, () => html`
				<menu class="gaa-data-path-autocomplete-dropdown gaa-dropdown-menu">
					${this._filteredDataPaths.map(dp => html`<li @mousedown=${() => this.#selectDatapath(dp)}>${formatPathString(dp.path)}</li>`)}
				</menu>
			`)}
		`;
	}
